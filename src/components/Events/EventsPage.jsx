    ...
    <>
      <main className="flex-1 rounded p-4 min-h-[60vh]">
        <Routes>
          ...
          <Route path={`${EVENT_CAT.B_MONDAYS.path}/nuevo`} element={<BankingMondaysForm year={new Date().getFullYear()} />} />
          <Route path={`${EVENT_CAT.B_MONDAYS.path}/ver`} element={<BankingMondaysForm mode="view" year={new Date().getFullYear()} />} />
          <Route path={`${EVENT_CAT.B_MONDAYS.path}/edit`} element={<BankingMondaysForm mode="edit" year={new Date().getFullYear()} />} />
        </Routes>
      </main>
    </>
  );
