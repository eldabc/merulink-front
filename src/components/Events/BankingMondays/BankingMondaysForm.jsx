...
export default function BankingMondaysForm({ mode = 'create', event = [], onBack, year }) {
  ...
  const { createEditBankingEvents, loadEvents, eventData, setEventData, loading, setLoading } = useEvents();
  ...
  useEffect(() => {
    if (!loading && Array.isArray(eventData)) {
  ...
  return (
    <div className="flex flex-col h-full text-white w-full p-2">
        {(viewMode) && <HeadFormButtons url="/eventos/lunes-bancarios/edit" data={eventData} /> }
      ...
      <div className="table-container overflow-y-auto max-h-[500px] pr-2 custom-scrollbar">
        {loading ? (
          <SpanText text="Cargando..." colSpan={6} />
        ) : (
          yearMondays.map((date) => {
            ...
            const initialValue = eventData?.find(e => e.start?.startsWith(dateStr))?.title || "";
