    ...
    <div className="absolute right-0 mt-2 w-80 bg-[#3c4042] border border-[#ffffff21] rounded-xl shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in duration-200">
      ...
      <ul className="max-h-96 overflow-y-auto">
        {hasNotifications ? (
          ...
          <li className="p-4 border-b border-[#ffffff05] hover:bg-[#ffffff05] transition-colors cursor-pointer">
            <p className="mr-2 text-sm text-gray-500 text-center">Sin Notificaciones</p>
          </li>
        )}
      </ul>
      <div className="p-2 text-center bg-[#4e5051]">
        {hasNotifications && <a className="text-xs hover:text-white">Ver todas</a> }
      </div>
    </div>
  );
}