...
function ToggleAutofill({ isOpen, compact = false, checkboxChecked, onCheckboxChange, onLoadingHandleAutofill }) {
  ...
  const toggleMessagge = checkboxChecked ? 'Desactivar' : 'Activar';
  return (
    <>
      {onLoadingHandleAutofill ? (
        <SpanText text="Cargando..." />
      ) : (
        <label className={wrapperClasses} title={`${toggleMessagge} generación automática de horarios cada quincena`}>
          <div className={compact ? 'flex items-center gap-2' : 'flex flex-col gap-0.5'}>
            <span className={`text-xs font-medium text-gray-200`}>
              {compact ? 'Relleno automático' : 'Rellenar siempre automáticamente' }
            </span>
            {!compact && (
              <span className="text-xs text-gray-400">
                Si marca esta opción no necesitará volver a crear futuras quincenas en este departamento.
              </span>
            )}
          </div>
